...
function createUI()
{
    ...
    this.newSlider("drawFrameBlending",10,255, 30);
}

//--------------------------------------------------------------
function eventUI(name, value)
{
	if (name == "drawLinesWidth")
	{
		this.drawLinesWidth = value;
	}
	else if (name == "drawFrameBlending")
	{
		this.drawFrameBlending = value;
	}
}
...
function draw(w,h)
{
	...
	of.SetColor(0,0,0,this.drawFrameBlending);
	of.Rect(0,0,w,h);
	of.SetLineWidth( this.drawLinesWidth );
	for (var i=0 ; i < this.particles.length ; i++){
